...
export type OperationContext = { interactionId?: string; actorUserId?: string; actorDiscordUserId: string; guildId?: string; channelId?: string };

export async function beginOperation(context: OperationContext, operationType: string, reason = "") {
  const id = crypto.randomUUID();
  const result = await env.DB.prepare("INSERT OR IGNORE INTO operation_requests (id, interaction_id, actor_user_id, actor_discord_user_id, guild_id, channel_id, operation_type, status, reason) VALUES (?, ?, ?, ?, ?, ?, ?, 'received', ?)").bind(id, context.interactionId ?? null, context.actorUserId ?? context.actorDiscordUserId, context.actorDiscordUserId, context.guildId ?? null, context.channelId ?? null, operationType, reason).run();
