...
export async function GET(request: Request) {
  ...
  try {
    ...
    const memberResponse = await fetch(`https://discord.com/api/users/@me/guilds/${value.guildId}/member`, { headers: { authorization: `Bearer ${token.access_token}` } }); if (memberResponse.status === 404) throw new Error("対象のオンライン麻雀大会サーバーへ参加してから再度ログインしてください"); if (!memberResponse.ok) throw new Error("Discordサーバー所属を確認できませんでした"); const member = await memberResponse.json() as { nick?: string | null };
    const userId = user.id; await env.DB.prepare("INSERT INTO users (id, discord_user_id, discord_username, discord_nickname) VALUES (?, ?, ?, ?) ON CONFLICT(discord_user_id) DO UPDATE SET discord_username = excluded.discord_username, discord_nickname = excluded.discord_nickname, updated_at = CURRENT_TIMESTAMP").bind(userId, user.id, user.global_name || user.username || user.id, member.nick || "").run();
    const savedUser = await env.DB.prepare("SELECT id FROM users WHERE id = ?").bind(user.id).first<{ id: string }>(); if (!savedUser) throw new Error("ユーザー保存に失敗しました");
