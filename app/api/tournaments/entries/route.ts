...
export async function POST(request: Request) {
  ...
  const tournament = await env.DB.prepare("SELECT rounds, phase, password FROM tournaments WHERE id = ?").bind(tournamentId).first<{ rounds: number; phase: string; password: string }>();
