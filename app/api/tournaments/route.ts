...
export async function GET(request: Request) {
  ...
  const isOrganizer = Boolean(profile?.nickname && profile.nickname === tournament.owner);
...
export async function POST(request: Request) {
  ...
  const owner = (await getProfile(request))?.nickname;
  if (!owner) return Response.json({ error: "先に利用者登録をしてください" }, { status: 401 });
  ...
  const statements = [db.prepare("INSERT INTO tournaments (id, owner, name, game_type, start_at, password, rounds, pairing_mode, uma, notice, phase) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'before')").bind(id, owner, name, gameType, startAt, password, rounds, pairingMode, JSON.stringify(body.uma), notice), ...defaultRounds(rounds).map((round) => db.prepare("INSERT INTO tournament_rounds (tournament_id, round, status, state_json, version) VALUES (?, ?, ?, ?, ?)").bind(id, round.round, round.status, JSON.stringify({ tables: [] }), 0))];
...
export async function PUT(request: Request) {
  ...
  const result = await db.prepare("UPDATE tournaments SET name = ?, game_type = ?, password = COALESCE(?, password), rounds = ?, pairing_mode = ?, notice = ? WHERE id = ? AND owner = ? AND phase = 'before'").bind(name, gameType, password, rounds, pairingMode, asText(body.notice), id, profile.nickname).run();
