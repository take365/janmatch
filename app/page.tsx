...
export default function TournamentListPage() {
  ...
  useEffect(() => { let active = true; window.localStorage.removeItem("janmatch:tournaments"); fetch("/api/tournaments").then((response) => response.ok ? response.json() : Promise.reject()).then((remote: Tournament[]) => { if (active) setTournaments(remote); }).catch(() => { if (active) setTournaments([]); }); setCurrentNickname(window.localStorage.getItem("janmatch:nickname") ?? ""); return () => { active = false; }; }, []);
